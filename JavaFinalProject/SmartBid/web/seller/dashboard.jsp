...
<%@ page session="true" %>
<%
    // Disable browser cache to always get fresh wallet balance
    ...
    // Wallet info
    double wallet = 0.0;
    PreparedStatement psWallet = conn.prepareStatement("SELECT balance FROM seller_wallet WHERE seller_id = ?");
    ...
    // Seller's own approved products
    PreparedStatement ps = conn.prepareStatement("SELECT id, name FROM products WHERE seller_id=? AND status='approved'");
    ps.setInt(1, uid);
...
<body class="bg-gray-100 min-h-screen py-8 px-4 md:px-12">
    <div class="max-w-7xl mx-auto bg-white p-8 rounded-xl shadow-xl space-y-10">
        <!-- Header Section -->
        <div class="flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-pink-100 to-yellow-100 p-6 rounded-xl shadow mb-6">
            ...
            <div class="text-center md:w-1/2">
                <h1 class="text-3xl font-bold text-gray-900">
                    Welcome, <span class="text-pink-600"><%= name %></span>
                </h1>
                <p class="text-gray-600 text-sm">Your product & auction control center</p>
            </div>
        </div>
            ...
            <a href="viewMyProducts.jsp"
               class="flex flex-col items-center justify-center bg-purple-500 text-white rounded-xl py-6 shadow hover:bg-purple-600 transition">
                ...
                My Products
                        ...
                        <!-- Add Product -->
            <div class="bg-blue-100 border-l-4 border-blue-500 p-5 rounded-lg shadow flex flex-col">
                <h2 class="text-lg font-semibold text-blue-800 mb-2">Add New Product</h2>
                <a href="addProduct.jsp"
                   class="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow w-max">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"
                         viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                    Upload Product
                </a>
